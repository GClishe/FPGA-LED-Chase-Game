...
architecture RTL of Game_Loop is
    type t_SM_Main is (START,           -- Start state. Wait for reset
                       ...
                       CYCLE,           -- Main gameplay state. LEDs cycle right-to-left, waiting for player input
    ...
    signal r_score : unsigned(3 downto 0) := "0000";
        ...
        -- state machine logic
            case r_curr_state is
                ...
                when INCR_SCORE =>
                    ...
                    if r_score = to_unsigned(WIN_SCORE - 1, r_score'length) then
                        r_cycle_vectorRL <= "0000000000000001";
                        r_cycle_vectorLR <= "1000000000000000";
                ...
                when WIN =>
                    -- in this state, I want to do something fun with the LEDs. Maybe we'll start a left to right AND right to left cycle at the same time, but cycle faster than in the CYCLE state?
